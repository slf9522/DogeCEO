...
def main():
    ...
    while True:
        ...
        print("Search: ")
        for tweet in tweets:
            tweetDate = datetime.strptime(tweet['created_at'][:-1],
            ...
            if tweetDate > lastDate:  # check if tweet has already been analyzed based on time of last tweet analyzed
                ...
                with open('cryptos.json') as cryptoJsonFile:
                    ...
                    for crypto in cryptoData['cryptos']:  # iterate through each cryto
                        for word in splitTweetContent:  # iterate through each word in the tweet
                            if similar(word, crypto['symbol'].lower()) > 0.75 or similar(word, crypto[
                                'name'].lower()) > 0.65:  # check if word is similar to crypto symbol or name
                                cryptoMessage = "crypto found: " + crypto['name'] + " in Elon's tweet: " + tweet[
                    ...
                    for stock in stockData['popular_stocks']:  # iterate through each stock
                        for word in splitTweetContent:  # iterate through each word in the tweet
                            if similar(word, stock['name'].lower()) > 0.75:  # check if word is similar to stock name
                                stockMessage = "stock found: " + stock['name'] + " in Elon's tweet: " + tweet[
                                    'text']  # generate stock found message
                                print(stockMessage)
                                # #send SMS with Twilio
                                # client.messages.create(
                                #     body=cryptoMessage,
                ...
                try:
                    if "attachments" in tweet:  # if tweet potentially contains images
                        requestURL = "https://api.twitter.com/2/tweets?ids=" + tweet[
                            'id'] + "&expansions=attachments.media_keys&media.fields=duration_ms,height,media_key,preview_image_url,public_metrics,type,url,width"  # send request to get tweet by tweet ID
                        requestTweet = requests.get(requestURL,
                                                    headers={"Authorization": "Bearer {}".format(bearer_token)})
                        for media in requestTweet.json()['includes']['media']:  # iterate through media array of tweet
                            imageURL = media['url']
                            urllib.request.urlretrieve(imageURL,
                                                       "tweeted-image.jpg")  # retrieve and download image from media
                            time.sleep(5)
